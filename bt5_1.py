...
# Универсальная функция отправки сообщений в личку и группу
def send_to_user_and_group(user_id: int, chat_id: Optional[int], text: str, reply_to: Optional[types.Message] = None, **kwargs):
    if reply_to:
        ...
        for resource, prices in data.items():
            ...
            if not existing:
                try:
                    ...
                    send_to_user_and_group(user_id, chat_id, f"❌ Ошибка при сохранении данных для {resource}.", message)
                    continue

        if saved_count > 0:
            send_to_user_and_group(user_id, chat_id, f"✅ Сохранено {saved_count} записей рынка.", message)
            ...
            for resource in EMOJI_TO_RESOURCE.values():
                if len(get_recent_data(resource, 15)) >= 2:
                    send_resource_selection(user_id, chat_id)
                    break
            ...
            send_to_user_and_group(user_id, chat_id, "ℹ️ Данные уже были сохранены ранее.", message)

    except Exception as e:
        logger.error(f"Ошибка при обработке сообщения рынка: {e}", exc_info=True)
        send_to_user_and_group(message.from_user.id, None, f"❌ Произошла ошибка: {str(e)}. Пожалуйста, попробуйте снова или свяжитесь с поддержкой.", message)
...
def send_resource_selection(user_id: int, chat_id: Optional[int] = None):
    ...
    try:
        username = bot.get_chat(user_id).username or 'User'
    except:
        username = 'User'
    message_text = f"📊 @{username}, выберите ресурс для отслеживания:"

    send_to_user_and_group(user_id, chat_id, message_text, reply_markup=keyboard)
...
def process_resource_selection(call):
    ...
    if len(records) < 2:
        send_to_user_and_group(call.from_user.id, call.message.chat.id if call.message.chat.type in ['group', 'supergroup'] else None, 
                               f"⚠️ Для {resource} недостаточно данных. Пришлите еще обновления рынка.", reply_to=call.message)
        return
    ...
    send_to_user_and_group(user_id, chat_id, message_text, reply_markup=keyboard, reply_to=call.message)
...
def cancel_action(call):
    ...
    send_to_user_and_group(user_id, call.message.chat.id if call.message.chat.type in ['group', 'supergroup'] else None, 
                           "❌ Действие отменено.", reply_to=call.message)
...
def process_direction_selection(call):
    ...
    if (direction == "down" and trend != "down") or (direction == "up" and trend != "up"):
        ...
        send_to_user_and_group(user_id, chat_id, message_text, reply_to=call.message)
...
def process_target_price(message):
    ...
    try:
        ...
        if target_price <= 0:
            send_to_user_and_group(user_id, chat_id, "❌ Цена должна быть положительным числом.", message)
            return
    except ValueError:
        send_to_user_and_group(user_id, chat_id, "❌ Пожалуйста, введите корректное число (например: 0.55).", message)
        return
    ...
    records = get_recent_data(resource, 15)
    if len(records) < 2:
        send_to_user_and_group(user_id, chat_id, "⚠️ Недостаточно данных для расчета скорости. Пришлите еще обновления рынка.", message)
        user_states.pop(user_id, None)
        user_data.pop(user_id, None)
        return
    ...
    speed = calculate_speed(records, "buy")
    if speed is None:
        send_to_user_and_group(user_id, chat_id, "⚠️ Не удалось рассчитать скорость изменения цены.", message)
        user_states.pop(user_id, None)
        user_data.pop(user_id, None)
        return
    ...
    if (direction == "down" and target_price >= adjusted_buy) or \
       (direction == "up" and target_price <= adjusted_buy):
        ...
        send_to_user_and_group(user_id, chat_id, message_text, message)
        return
    ...
    threading.Thread(target=schedule_alert, args=(alert_id, user_id, resource, target_price, alert_time, chat_id, message_id), daemon=True).start()

    user_states.pop(user_id, None)
    user_data.pop(user_id, None)

# Фоновая задача для отправки уведомления
def schedule_alert(alert_id: int, user_id: int, resource: str, target_price: float, alert_time: datetime, chat_id: Optional[int] = None, message_id: Optional[int] = None):
    now = datetime.now()
...
def cmd_history(message):
    try:
        ...
        current_hour = None
        for record in records[-10:]:  # Последние 10 записей
...
# Команда /timer [ресурс] [цель цены]
...
# Универсальная функция обновления динамических таймеров (объединена с check_trend_changes)
def update_dynamic_timers_once():
    _update_dynamic_timers(single_run=True)

def _update_dynamic_timers(single_run: bool = False):
    try:
        Alert = Query()
...
# Фоновая задача для динамического пересчета таймеров
def update_dynamic_timers():
    while True:
        _update_dynamic_timers()
    ...
    while True:
        try:
            ...
            for user_id, alerts in user_alerts.items():
                ...
                enabled_users.append((user_id, alerts, settings))

            if not enabled_users:
                time.sleep(300)  # 5 минут, если нет пользователей с включенными напоминаниями
                continue

            for user_id, alerts, settings in enabled_users:
                push_interval = settings['push_interval']
                ...
                for g_chat_id in group_chats:
                    try:
                        bot.send_message(g_chat_id, reminder_text)
                    except Exception as e:
                        logger.error(f"Не удалось отправить напоминание в групповой чат {g_chat_id}: {e}")
                        bot.send_message(user_id, f"⚠️ Не удалось отправить напоминание в групповой чат {g_chat_id}.")

            # Ожидание до следующей проверки (минимальный интервал для всех пользователей)
            min_interval = min(settings['push_interval'] for _, _, settings in enabled_users)
